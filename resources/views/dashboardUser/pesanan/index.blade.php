        ...
        <tbody>
          @foreach ($pesanans as $pesanan)
           <tr>
            ...
            <td>{{ $seller->firstWhere('id', $pesanan->id_seller)->name}}</td>
            <td>Rp. {{ $produks->firstWhere('id_produk', $pesanan->id_produk)->harga}}</td>
            <td>
              <a href="/dashboardUser/riwayat/{{$pesanan->id_pesanan}}/edit" class="badge bg-warning">
                <i>test</i>  
              </a>
              ...
              <button class="badge bg-danger" onclick="return confirm('Anda yakin ingin menghapus produk?')">
              </button>
              </form>
            </td>
          </tr>   
